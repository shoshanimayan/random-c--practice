...
public:
    ...
    std::string value(const std::string& key) const; //returns from bucket

    // size() returns the number of key/value pairs stored in this HashMap.
    unsigned int size() const; // returns size vriable

    // bucketCount() returns the number of buckets currently allocated in
    // this HashMap.
    unsigned int bucketCount() const; //return bucketnum  variable
    ...
    double loadFactor() const; //return size/bucketnumber

    // maxBucketSize() returns the number of key/value pairs stored in this
    // HashMap's largest bucket.
    unsigned int maxBucketSize() const; //return largest bucket?
...
private:
    ...
    // check to see if n** is a pointer to a pointer, is the hashtable a pointer to a pointer?
    unsigned int bucketNumVariable;
    Node** OuterArray;
    std::string defaultKey;
    std::string defaultVal;
    
    unsigned int sizeVariable;
    void Nodetest(Node a);
    void bucketIdentitySteal(int theLength, int theSize, Node** TheOld, Node** theNew) const;
